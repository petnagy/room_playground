...
class SqliteDatabaseHandler(context: Context): SQLiteOpenHelper(context, Database.NAME, null, Database.VERSION) {
    ...
    override fun onCreate(sqLiteDatabase: SQLiteDatabase) {
        ...
        val createSwCharTable = "CREATE TABLE " + Database.TABLE_SW_CHARS +
                ...
                SwChar.SIDE + " INTEGER, " +
                SwChar.ROLE + " TEXT" + ")"
        sqLiteDatabase.execSQL(createSwCharTable)
    ...
    fun insertMainChars(sqLiteDatabase: SQLiteDatabase) {
        ...
        val insertText = "INSERT INTO ${Database.TABLE_SW_CHARS} (${SwChar.ID}, ${SwChar.NAME}, ${SwChar.RACE}, ${SwChar.SIDE}, ${SwChar.ROLE}) VALUES "

        val insert1 = insertText + "(${LUKE_SKYWALKER.id}, '${LUKE_SKYWALKER.name}', ${LUKE_SKYWALKER.race}, ${LUKE_SKYWALKER.side}, '${LUKE_SKYWALKER.role}')"
        val insert2 = insertText + "(${LEIA.id}, '${LEIA.name}', ${LEIA.race}, ${LEIA.side}, '${LEIA.role}')"
        val insert3 = insertText + "(${HAN_SOLO.id}, '${HAN_SOLO.name}', ${HAN_SOLO.race}, ${HAN_SOLO.side}, '${HAN_SOLO.role}')"
        val insert4 = insertText + "(${CHEWBACCA.id}, '${CHEWBACCA.name}', ${CHEWBACCA.race}, ${CHEWBACCA.side}, '${CHEWBACCA.role}')"
        val insert5 = insertText + "(${LANDO.id}, '${LANDO.name}', ${LANDO.race}, ${LANDO.side}, '${LANDO.role}')"
        val insert6 = insertText + "(${R2D2.id}, '${R2D2.name}', ${R2D2.race}, ${R2D2.side}, '${R2D2.role}')"
        val insert7 = insertText + "(${C3PO.id}, '${C3PO.name}', ${C3PO.race}, ${C3PO.side}, '${C3PO.role}')"
        val insert8 = insertText + "(${DART_VADER.id}, '${DART_VADER.name}', ${DART_VADER.race}, ${DART_VADER.side}, '${DART_VADER.role}')"
        val insert9 = insertText + "(${PALPATIN.id}, '${PALPATIN.name}', ${PALPATIN.race}, ${PALPATIN.side}, '${PALPATIN.role}')"
        val insert10 = insertText + "(${OBI_WAN.id}, '${OBI_WAN.name}', ${OBI_WAN.race}, ${OBI_WAN.side}, '${OBI_WAN.role}')"
        val insert11 = insertText + "(${BOBA_FETT.id}, '${BOBA_FETT.name}', ${BOBA_FETT.race}, ${BOBA_FETT.side}, '${BOBA_FETT.role}')"
        val insert12 = insertText + "(${JABBA.id}, '${JABBA.name}', ${JABBA.race}, ${JABBA.side}, '${JABBA.role}')"
        val insert13 = insertText + "(${TARKIN.id}, '${TARKIN.name}', ${TARKIN.race}, ${TARKIN.side}, '${TARKIN.role}')"
        val insert14 = insertText + "(${MOTTI.id}, '${MOTTI.name}', ${MOTTI.race}, ${MOTTI.side}, '${MOTTI.role}')"
    ...
    companion object {
        fun insertMainChars(sqLiteDatabase: SupportSQLiteDatabase) {
            val LUKE_SKYWALKER = StarWarsChar(1, "Luke Skywalker", Race.HUMANOID.race, Side.REBEL.side, "Jedi")
            val LEIA = StarWarsChar(2, "Leia Organa", Race.HUMANOID.race, Side.REBEL.side, "general")
            val HAN_SOLO = StarWarsChar(3, "Han Solo", Race.HUMANOID.race, Side.REBEL.side, "smuggler")
            val CHEWBACCA = StarWarsChar(4, "Chewbacca", Race.WOOKIE.race, Side.REBEL.side, "co-pilot")
